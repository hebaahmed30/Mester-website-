...
import { ADDSTUDENTTOCOURSE, BASEURL } from "../API/API";
...
function AddStudentToCourse() {
  ...
  useEffect(() => {
    document.title = " إضافة الكورس الي الطالب";
    return () => {
      ...
      <form onSubmit={handleSubmit} className="my-3 text-right m-4 md:m-0">
        ...
        <div className=" mt-6  md:mb-0 ">
          <div className="text-right ">
            <button
              className={` rounded-[9px] shadow-xl px-4 lg:px-20 py-2 text-white hover:border-amber-400  hover:bg-white transition duration-700  hover:text-black ${
                isDarkMode ? "bg-sky-400" : "bg-gray-800"
              }`}
            >
              إضافة الكورس
            </button>
